# Android TV — per-run feature log

This file is the archive of every Run 12-19 feature note for the
Android TV client. It exists so that the top-level
`apps/android-tv/README.md` can stay slim while still preserving the
detail-rich per-feature documentation produced during Phase C / D.

If you're new here, start with `apps/android-tv/README.md` for the
project layout, build commands, and design system pointer. Come back
here for the per-feature deep dives.

> The Run-by-Run roadmap, locked decisions, and current state live in
> the repo-root `CLAUDE.md`. This file mirrors only what was shipped
> inside `apps/android-tv/`.
